...
token = "OTQzNDEzNzc3NjAzMzc5MjUx.YgysjA.96aDppnjwvglMCSXRTdw4TsYYRk"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 50):
      ...
      embes = discord.Embed(title="Vespy Hub", description="FREE SMS SPAM",color=0xff4612)
      embes.add_field(name="ยิงไปที่เบอร์",value=phone)
      ima = "https://cdn.discordapp.com/attachments/931454415544651818/949172160331653160/standard.gif"
      ...
      BBot(phone,amount)
    	...
    	await ctx.channel.send("```ไม่เกิน 50 ดิสุดหล่อ```")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="Vespy Hub",description="วิธิใช้",color=0xff4612)
